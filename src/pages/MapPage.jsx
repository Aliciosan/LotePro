...
export default function MapPage() {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 h-[calc(100vh-180px)] md:h-[calc(100vh-100px)] animate-fade-in relative">
      ...
      {/* Drawer Overlay */}
      {selectedLot && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-[2px] z-40 transition-opacity duration-300" onClick={closeDrawer}></div>
      )}

      {/* Drawer Panel */}
      <div className={`fixed inset-y-0 right-0 w-full md:w-[450px] bg-white shadow-2xl z-50 transform transition-transform duration-300 flex flex-col md:rounded-l-3xl mt-10 md:mt-0 rounded-t-3xl md:rounded-t-none h-[calc(100%-40px)] md:h-full top-auto bottom-0 md:top-0 ${selectedLot ? 'translate-x-0' : 'translate-x-full'}`}>
        {selectedLot && (
          <div className="h-full flex flex-col relative bg-white">
            <button onClick={closeDrawer} className="absolute top-4 right-4 z-20 bg-white/20 backdrop-blur p-2 rounded-full text-white hover:bg-white hover:text-navy transition-colors shadow-lg">
                <X size={20} />
            </button>
            
            <div className="h-64 md:h-72 relative shrink-0">
               <img src="https://images.unsplash.com/photo-1605276374104-dee2a0ed3cd6?q=80&w=800&auto=format&fit=crop" className="w-full h-full object-cover md:rounded-tl-3xl" />
               <div className="absolute bottom-0 w-full bg-gradient-to-t from-navy to-transparent p-6 pt-16">
            ...
            <div className="p-6 border-t border-gray-100 bg-white md:rounded-bl-3xl pb-safe">
